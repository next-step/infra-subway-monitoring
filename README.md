...
### 요구사항
- [x] 부하 테스트
    - [x] 테스트 전제조건 정리 ➝ 하기 [테스트 계획하기](#테스트_계획하기)를 통해 확인 가능
        - [x] 대상 시스템 범위
        - [x] 목푯값 설정 (latency, throughput, 부하 유지기간)
        - [x] 부하 테스트 시 저장될 데이터 건수 및 크기
    - [x] 아래 시나리오 중 하나 선택하여 스크립트 작성
        - 접속 빈도가 높은 페이지
            - 메인 페이지
        - 데이터를 갱신하는 페이지
        ...
        - [x] 데이터를 조회하는데 여러 데이터를 참조하는 페이지
            ...
            - 스크립트 및 시나리오는 `docs/README.md`에서 확인 가능
    - [x] smoke, Load, Stress 테스트 후 결과 기록
      - `docs/smoke/*`, `docs/load/*`, `docs/stress/*` 패키지 하위에 K6 로그 및 그라파나 결과 기록
