...
3. 부하테스트 전제조건은 어느정도로 설정하셨나요
   ...
   > Throughput ->   
    1일 사용자 수(DAU) x 1명당 1일 평균 접속 수 = 1일 총 접속 수 : 200  * 2 = 400 (1일 총 접속 수)    
    1일 총 접속 수 / 86,400 (초/일) = 1일 평균 rps : 400 / 86,400 = 0.0046 (1일 평균rps)  
    1일 평균 rps x (최대 트래픽 / 평소 트래픽) = 1일 최대 rps : 0.0046 * (?/?) = ?  
   
   > ⁉ 제시되어 있는대로 Throughput 값을 구해보려 했는데, 알 수 없는 부분도 있고, Request per second(rps) 값을
     구한다 한들 이 부하 테스트 전제 조건하고 매칭이 잘 안되어서 k6 부하테스트 요건으로 정의해보았습니다. 
              
가상유저 약 200명이 3초 이내(기준 2000ms)에 페이지 로딩이 완료되고, duration 30초 동안 요청 테스트(성능유지기간), 
99프로의 성공률을 기대 하는 것으로 설정하였습니다.  
- 테스트 시나리오 : 
  - 메인 페이지 접속  
  - 로그인하기
  - 내 정보 확인하기
