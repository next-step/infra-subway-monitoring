...
public class PerformanceTimeTracer {
    ...
    private static final Logger performanceTimeTraceLogger = LoggerFactory.getLogger(LogName.CONSOLE.logName());

    @Around("within(nextstep.subway.auth.ui..*)")
    public Object calculatePerformanceTimeAuth(ProceedingJoinPoint joinPoint) throws Throwable {
        return object(joinPoint);
    }

    @Around("within(nextstep.subway.favorite.ui..*)")
    public Object calculatePerformanceTimeFavorite(ProceedingJoinPoint joinPoint) throws Throwable {
        return object(joinPoint);
    }

    @Around("within(nextstep.subway.line.ui..*)")
    public Object calculatePerformanceTimeLine(ProceedingJoinPoint joinPoint) throws Throwable {
        return object(joinPoint);
    }

    @Around("within(nextstep.subway.map.ui..*)")
    public Object calculatePerformanceTimeMap(ProceedingJoinPoint joinPoint) throws Throwable {
        return object(joinPoint);
    }
