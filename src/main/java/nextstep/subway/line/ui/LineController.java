...
public class LineController {
    ...
    @PostMapping
    @SubwayLogging(description = "신규 노선 생성(/lines)")
    ...
    @GetMapping
    @SubwayLogging(description = "모든 지하철 노선 조회(/lines)")
    public ResponseEntity<List<LineResponse>> findAllLines() {
        return ResponseEntity.ok(lineService.findLineResponses());
    }

    @GetMapping("/{id}")
    @SubwayLogging(description = "아이디 기준 지하철 노선 조회(/lines/{id})")
    public ResponseEntity<LineResponse> findLineById(@PathVariable Long id) {
        return ResponseEntity.ok(lineService.findLineResponseById(id));
    }

    @PutMapping("/{id}")
    @SubwayLogging(description = "아이디 기준 지하철 노선 수정(/lines/{id})")
    ...
    @DeleteMapping("/{id}")
    @SubwayLogging(description = "아이디 기준 지하철 노선 삭제(/lines/{id})")
    ...
    @SubwayLogging(description = "지하철 노선 구간 추가(/lines//{lineId}/sections)")
    ...
    @SubwayLogging(description = "지하철 노선 구간 삭제(/lines//{lineId}/sections)")
